...
#define emptymemory_TtaskDefault 5000
...
public:
    ...
    bool is_all_del(bool force = false)
    {
        ...
        if (b_NET)
        {
            rsdebugInfF("DEF: Очищаем память от сетевых настроек");
            rsdebugInfln("$%d", len_ethernet_settings_ROM);
        }
        if (b_SYS)
        {
            rsdebugInfF("DEF: Очищаем память от системных настроек");
            rsdebugInfln("$%d", len_sys_settings_ROM);
        }
        if (b_NTP)
        {
            rsdebugInfF("DEF: Очищаем память от настроек NTP");
...
public:
    ...
    bool is_all_del(bool force = false)
    {
        ...
        if (b_NET)
        {
            rsdebugInfF("ROM: Очищаем память от сетевых настроек");
            rsdebugInfln("$%d", len_ethernet_settings_ROM);
        }
        if (b_SYS)
        {
            rsdebugInfF("ROM: Очищаем память от системных настроек");
            rsdebugInfln("$%d", len_sys_settings_ROM);
        }
        if (b_NTP)
        {
            rsdebugInfF("ROM: Очищаем память от настроек NTP");
            rsdebugInfln("$%d", len_NTP_settings_ROM);
        }
        return b_NET & b_SYS & b_NTP;
    };
...
public:
    ...
    bool is_all_del(bool force = false)
    {
        ...
        if (b_NET)
        {
            rsdebugInfF("RAM: Очищаем память от сетевых настроек");
            rsdebugInfln("$%d", len_ethernet_settings_ROM);
        }
        if (b_SYS)
        {
            rsdebugInfF("RAM: Очищаем память от системных настроек");
            rsdebugInfln("$%d", len_sys_settings_ROM);
        }
        if (b_NTP)
        {
            rsdebugInfF("RAM: Очищаем память от настроек NTP");
            rsdebugInfln("$%d", len_NTP_settings_ROM);
        }
        return b_NET & b_SYS & b_NTP;
    };
